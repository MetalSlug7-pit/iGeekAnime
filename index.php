...
include_once('php/criaTabelas.php');

$resultSlidesUltimaAdd = $conn->query("SELECT * FROM Obras ORDER BY dataLancamento DESC LIMIT 1");
$resultSlidesPopular = $conn->query("SELECT idObra, count(idObra) from Favoritar_obra_usuario group by idObra order by count(idObra) DESC limit 1");


$resultUltimaAdd = $conn->query("SELECT * FROM Obras ORDER BY dataLancamento DESC LIMIT 5");
$resultPopular = $conn->query("SELECT idObra, count(idObra) from Favoritar_obra_usuario group by idObra order by count(idObra) DESC LIMIT 5");

$resultObrasExp = $conn->query("SELECT obras_id from Obras where obras_id not in (SELECT idObra from Favoritar_obra_usuario)");

$linhas = $resultObrasExp->rowCount();

if($linhas>0){
	$resultSlidesExplorar = $conn->query("SELECT obras_id from Obras where obras_id not in (SELECT idObra from Favoritar_obra_usuario) LIMIT 1");
}
else{
	$resultSlidesExplorar = $conn->query("SELECT idObra, count(idObra) from Favoritar_obra_usuario group by idObra order by count(idObra) ASC limit 1");
}

$limite = 5 - $linhas;

if($limite>0){
	$resultExplorar = $conn->query("SELECT idObra, count(idObra) from Favoritar_obra_usuario group by idObra order by count(idObra) ASC LIMIT $limite");
}
else{
	$linhasExplorar = 1;
}
...
$linhasSlidesUltimaAdd = $resultSlidesUltimaAdd->rowCount();
$linhasUltimaAdd = $resultUltimaAdd->rowCount();

$linhasPopular = $resultPopular->rowCount();
if($limite>0){
	$linhasExplorar = $resultExplorar->rowCount();
}
$i=0;

while($row_obras = $resultSlidesUltimaAdd->fetch(PDO::FETCH_ASSOC)){
	$imgsSlidesUltimaAdd[] = $row_obras;
}

while($row_obras = $resultSlidesPopular->fetch(PDO::FETCH_ASSOC)){
	$imgsSlidesPopular[] = $row_obras;
}

while($row_obras = $resultSlidesExplorar->fetch(PDO::FETCH_ASSOC)){
	$imgsSlidesExplorar[] = $row_obras;
}

while($row_obras = $resultUltimaAdd->fetch(PDO::FETCH_ASSOC)){
	$imgsUltimaAdd[] = $row_obras;
}

while($row_obrasPop = $resultPopular->fetch(PDO::FETCH_ASSOC)){
	$imgsPopular[] = $row_obrasPop;
}

if($limite>0){
	while($row_obras = $resultExplorar->fetch(PDO::FETCH_ASSOC)){
		$imgsExplorar[] = $row_obras;
	}
}

while($row_obrasExplor = $resultObrasExp->fetch(PDO::FETCH_ASSOC)){
	$obExp[] = $row_obrasExplor;
}
?>
...
<body style="visibility: hidden !important; background-color: #171616;">
		  ...
		  <div class="collapse navbar-collapse" id="navbarSupportedContent">
			<ul class="navbar-nav me-auto mb-2 mb-lg-0 nav justify-content-center">
			  <li class="nav-item icone">
				<a class="nav-link" href="paginas/tudo.php">
			  ...
			  <li class="nav-item dropdown icone">
				...
				?>
					<ul class="dropdown-menu">
						<li><?php echo "<a href='paginas/perfil.php?id=" . $_SESSION['id'] . "'class='dropdown-item'>Perfil</a>";?></li>
				...
				?>
					<ul class="dropdown-menu">
						<li><a href='paginas/gerenciarObras.php' class="dropdown-item">Gerenciar Obras</a></li>
						<li><a href='paginas/gerenciarGeneros.php' class="dropdown-item">Gerenciar Gêneros</a></li>
				  		<li><a href='paginas/gerenciarAutores.php' class="dropdown-item">Gerenciar Autores</a></li>
				  		<li><a href="php/logout.php" class="dropdown-item">Sair</a></li>
					</ul>
			  </li>
				<?php
	...
	<section class="obras-section">
		<div class="obras-container">
			<?php 
			if($linhasSlidesUltimaAdd==0){					
			?>
					
			<?php	
			} else {
				foreach($imgsSlidesUltimaAdd as $obraSlideUltimaAdd){
			?>
			<div class="obraWall">
				<img src="<?php echo $obraSlideUltimaAdd['wallpaperObra'];?>" alt="<?php echo $obraSlideUltimaAdd['nome'];?>">
				<?php echo "<a href='paginas/obras/obras.php?id=". $obraSlideUltimaAdd['obras_id'] . "'>"?><div class="info-obra">
					<h3><?php echo $obraSlideUltimaAdd['nome'];?></h3>
					<p>
					<?php 
						$idObra = $obraSlideUltimaAdd['obras_id'];
						$resultGen = $conn->query("SELECT * FROM GENEROS_OBRA_GENERO WHERE idObra = '$idObra'");
						$generos = array();

						while($row_obra_genero = $resultGen->fetch(PDO::FETCH_ASSOC)){
						$generos[] = $row_obra_genero;
						}

						foreach($generos as $gen){
						$idGen = $gen['idGenero'];
						$buscaGen = $conn->query("SELECT * FROM GENERO WHERE genero_id = '$idGen'");
						$row_generos = $buscaGen->fetch(PDO::FETCH_ASSOC);
						echo $row_generos['tipoGenero'] . " ";
						}
					?>
					</p>
				</div></a>
			...
			<?php	
				foreach($imgsSlidesPopular as $obraSlidePopular){
					$idObra = $obraSlidePopular['idObra'];
					$resSlideObraPop = $conn->query("SELECT * FROM Obras where obras_id = '$idObra'");
					$row_slidePop = $resSlideObraPop->fetch(PDO::FETCH_ASSOC);
			?>
			<div class="obraWall">
				<img src="<?php echo $row_slidePop['wallpaperObra'];?>" alt="<?php echo $row_slidePop['nome'];?>">
				<?php echo "<a href='paginas/obras/obras.php?id=". $row_slidePop['obras_id'] . "'>"?><div class="info-obra">
					<h3><?php echo $row_slidePop['nome'];?></h3>
					<p>
					<?php 
						$idObra = $row_slidePop['obras_id'];
						$resultGen = $conn->query("SELECT * FROM GENEROS_OBRA_GENERO WHERE idObra = '$idObra'");
						$generos = array();

						while($row_obra_genero = $resultGen->fetch(PDO::FETCH_ASSOC)){
						$generoPop[] = $row_obra_genero;
						}

						foreach($generoPop as $gen){
						$idGen = $gen['idGenero'];
						$buscaGen = $conn->query("SELECT * FROM GENERO WHERE genero_id = '$idGen'");
						$row_generos = $buscaGen->fetch(PDO::FETCH_ASSOC);
						echo $row_generos['tipoGenero'] . " ";
			...
			<?php
				if($linhas>0){
					foreach($imgsSlidesExplorar as $obraSlideExplorar){
						$idObra = $obraSlideExplorar['obras_id'];
						$resSlideObraExp = $conn->query("SELECT * FROM Obras where obras_id = '$idObra'");
						$row_slideExp = $resSlideObraExp->fetch(PDO::FETCH_ASSOC);
			?>
				<div class="obraWall">
					<img src="<?php echo $row_slideExp['wallpaperObra'];?>" alt="<?php echo $row_slideExp['nome'];?>">
					<?php echo "<a href='paginas/obras/obras.php?id=". $row_slideExp['obras_id'] . "'>"?><div class="info-obra">
						<h3><?php echo $row_slideExp['nome'];?></h3>
						<p>
						<?php 
							$idObra = $row_slideExp['obras_id'];
							$resultGen = $conn->query("SELECT * FROM GENEROS_OBRA_GENERO WHERE idObra = '$idObra'");
							$generos = array();

							while($row_obra_genero = $resultGen->fetch(PDO::FETCH_ASSOC)){
							$generoExp[] = $row_obra_genero;
							}

							foreach($generoExp as $gen){
							$idGen = $gen['idGenero'];
							$buscaGen = $conn->query("SELECT * FROM GENERO WHERE genero_id = '$idGen'");
							$row_generos = $buscaGen->fetch(PDO::FETCH_ASSOC);
							echo $row_generos['tipoGenero'] . " ";
	...
	<?php 
		if($linhasUltimaAdd != 0 && $linhasPopular !=0 && $linhasExplorar !=0){
	?>
		<div class="container">
			<h2 style="color: white;">Últimas Adicionadas</h2>
			<a href='paginas/todasAdicionadas.php'><button class="btn btn-outline-light position-absolute" type="submit" style="margin-left: 1060px; margin-top: -45px;">Ver Todas</button></a>
			<div class="recomendacao">
				<div class="obras">
					<?php 
						if($linhasUltimaAdd == 0) {					
					?>
							<div class="obra">
								<img src="https://picsum.photos/id/1015/205/300" alt="Nenhuma obra encontrada">
								<h3>Nenhuma obra encontrada</h3>
							</div>
					<?php	
						}else {
							foreach($imgsUltimaAdd as $obraUltimaAdd) {
					?>		
								<div class="obra">
									<img src="<?php echo $obraUltimaAdd['imagemObra'];?>">
									<h3><?php echo $obraUltimaAdd['nome'];?></h3>
									<a href="paginas/obras/obras.php?id=<?php echo $obraUltimaAdd['obras_id'];?>" class="overlay"></a>
								</div>
						<?php 
							}
							}
		...
		<div class="container">
			...
			<a href='paginas/todasPopulares.php'><button class="btn btn-outline-light position-absolute" type="submit" style="margin-left: 1060px; margin-top: -45px;">Ver Todas</button></a>
			<div class="recomendacao">
				<div class="obras">
					<?php 
						if($linhasPopular == 0) {					
					?>
						...
						<?php	
								...
								foreach($imgsPopular as $obraPopular){
									$idObra = $obraPopular['idObra'];
									$resObrasPop = $conn->query("SELECT * FROM Obras where obras_id = '$idObra'");
									$row_pop = $resObrasPop->fetch(PDO::FETCH_ASSOC);
						?>		
								<div class="obra">
									<img src="<?php echo $row_pop['imagemObra'];?>">
									<h3><?php echo $row_pop['nome'];?></h3>
									<a href="paginas/obras/obras.php?id=<?php echo $row_pop['obras_id'];?>" class="overlay"></a>
								</div>
						<?php 
							}
							}
		...
		<div class="container">
		<h2 style="color: white;">Explorar</h2>
		<a href='paginas/todasExplorar.php'><button class="btn btn-outline-light position-absolute" type="submit" style="margin-left: 1060px; margin-top: -45px;">Ver Todas</button></a>
			<div class="recomendacao">
				<div class="obras">
					<?php 
						if($linhasExplorar == 0) {					
					?>
						...
						<?php	
								...
								if($linhas>0){
									foreach($obExp as $obraExplorar){
										if($i >= 5){
											break;
										}
										$idObraExplorar = $obraExplorar['obras_id'];
										$resObrasExp = $conn->query("SELECT * FROM Obras where obras_id = '$idObraExplorar'");
										$row_exp = $resObrasExp->fetch(PDO::FETCH_ASSOC);
										$i++;
						?>		
								<div class="obra">
									<img src="<?php echo $row_exp['imagemObra'];?>">
									<h3><?php echo $row_exp['nome'];?></h3>
									<a href="paginas/obras/obras.php?id=<?php echo $row_exp['obras_id'];?>" class="overlay"></a>
								</div>
						<?php 
							}
							}
							if($limite>0){
								foreach($imgsExplorar as $obraExplorar){
									$idObraExplorar = $obraExplorar['idObra'];
									$resObrasExp = $conn->query("SELECT * FROM Obras where obras_id = '$idObraExplorar'");
									$row_exp = $resObrasExp->fetch(PDO::FETCH_ASSOC);
						?>
									<div class="obra">
		...
		<div class="container-fluid">
			...
			<div class="container-fluid">
				...
				<div class="collapse navbar-collapse" id="navbarSupportedContent">
					<ul class="navbar-nav me-auto mb-2 mb-lg-0 nav justify-content-center">
						<li class="nav-item icone">
							<a class="nav-link" href="paginas/termosUso.php">
						...
						<li class="nav-item icone">
							<a class="nav-link" href="paginas/politicaPrivacidade.php">
			...
			<div class="container-fluid">
				...
				<div class="collapse navbar-collapse" id="navbarSupportedContent">
					<ul class="navbar-nav me-auto mb-2 mb-lg-0 nav justify-content-center">
						...
						<li class="nav-item icone">
							<a class="nav-link" href="paginas/faqs.php">
