...
$resultNome = $conn->query("SELECT * FROM Obras WHERE nome like '%$nome%' order by nome");
$linhas = $resultNome->rowCount();
while($row_obraBuscada = $resultNome->fetch(PDO::FETCH_ASSOC)){
	$obraBuscada[] = $row_obraBuscada;
}
?>
<!DOCTYPE html>
<html lang="pt-br">
		...
		<div class="recomendacao">
			<div class="obras">
				<?php 
					if($linhas == 0) {					
				?>
						<body class="zero" onload="obraNaoEncontrada()"></body>
				<?php	
					}else {
						foreach($obraBuscada as $obraBusc) {
				?>		
							<div class="obra">
								<img src="../<?php echo $obraBusc['imagemObra'];?>">
								<h3><?php echo $obraBusc['nome'];?></h3>
								<a href="obras/obras.php?id=<?php echo $obraBusc['obras_id'];?>" class="overlay"></a>
							</div>
					<?php 
						}
						}
