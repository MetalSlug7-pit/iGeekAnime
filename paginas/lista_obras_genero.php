...
if (isset($_GET['tipoGenero'])) {
    $tipoGenero = $_GET['tipoGenero'];
} else {

}

$resultObrasTipoGenero = $conn->query("SELECT genero_id FROM Genero WHERE tipoGenero = '$tipoGenero'");

$linhasObrasTipoGenero = $resultObrasTipoGenero->rowCount();

while($row_obras = $resultObrasTipoGenero->fetch(PDO::FETCH_ASSOC)){
	$obrasTipoGenero[] = $row_obras;
}

foreach($obrasTipoGenero as $obraTipoGenero){
	$idGen = $obraTipoGenero['genero_id'];
	$resultTipGen = $conn->query("SELECT * FROM Generos_Obra_Genero WHERE idGenero  = '$idGen'");
	while($row_generos = $resultTipGen->fetch(PDO::FETCH_ASSOC)){
		$obras[] = $row_generos;
	}
}

foreach($obras as $buscaObra){
	$idObra = $buscaObra['idObra'];
	$resultObra = $conn->query("SELECT * FROM obras where obras_id = '$idObra'");
	while($row_obra = $resultObra->fetch(PDO::FETCH_ASSOC)){
		$obrasF[] = $row_obra;
	}
}
?>
...
<body style="visibility: hidden !important; background-color: #171616;">
	...
	<nav class="navbar navbar-expand-lg bg-dark position-absolute" style="width: 100%;">
	...
	<div class="container">
		<h2 style="color: white;"><?php echo $tipoGenero;?></h2>
		<div class="recomendacao">
			<div class="obras">
				<?php 
					if($linhasObrasTipoGenero == 0) {					
				?>
						<div class="obra">
				...
				<?php	
						...
						foreach($obrasF as $obra) {
				?>		
							<div class="obra">
								<img src="../<?php echo $obra['imagemObra'];?>">
								<h3><?php echo $obra['nome'];?></h3>
								<a href="obras/obras.php?id=<?php echo $obra['obras_id'];?>" class="overlay">
