<?php
include('../php/verificaçãoLogadoPerfil.php');
include_once('../php/ConectBanco.php');
$id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT);
$result = $conn->query("SELECT * FROM Usuario WHERE usuario_id = '$id'");
$result->execute();
$row_usuario = $result->fetch(PDO::FETCH_ASSOC);

$idObras=null;

$resultIdObras = $conn->query("SELECT idObra FROM Favoritar_obra_usuario WHERE idUsuario = '$id'");
$linhas = $resultIdObras->rowCount();
while($row_idObras = $resultIdObras->fetch(PDO::FETCH_ASSOC)){
	$idObras[] = $row_idObras;
}
...
if($idObras==null){
	...
	foreach($idObras as $obra){
		$idObra = $obra['idObra'];
		$resultObra = $conn->query("SELECT * FROM OBRAS WHERE obras_id = '$idObra'");
		while($row_obras = $resultObra->fetch(PDO::FETCH_ASSOC)){
			$imgs[] = $row_obras;
		}
	}
}
...
<html lang="pt-br">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="../css/configUser.css"/>
...
<?php
			$diretorioImgWallpaper = "../imagens/contas/wallpapersPerfil";

			if(dirWallpaperVazio($diretorioImgWallpaper)){

				$srcWallpaper = "imagens/contas/wallpaperBasico.png";
			}
			else{
				$srcWallpaper = $row_usuario['fotoWallpaper'];
			}

			function dirWallpaperVazio($diretorioImgWallpaper){
				if(!is_readable($diretorioImgWallpaper)){
					return null;
				}
				return (count(scandir($diretorioImgWallpaper)) == 2);
			}
?>
...
<body style="visibility: hidden !important; background-color: #171616;">
	...
	<nav class="navbar navbarCima navbar-expand-lg bg-dark position-absolute" style="width: 100%;">
		...
		<?php
			$diretorioImgAvatar = "../imagens/contas/avataresPerfil/";
			if(dirAvatarVazio($diretorioImgAvatar)){

				$srcAvatar = "imagens/contas/avatarBasico.png";
			}
			else{
				$srcAvatar = $row_usuario['fotoPerfil'];
			}

			function dirAvatarVazio($diretorioImgAvatar){
				if(!is_readable($diretorioImgAvatar)){
					return null;
				}
				return (count(scandir($diretorioImgAvatar)) == 2);
			}
		?>
	
	<img class="imgPerfil" src="../<?php echo $srcWallpaper;?>">

	<ul class="nav nav-pills mb-3 nav-fill position-relative" id="pills-tab" role="tablist">
		<li class="nav-item">
		...
		<li class="nav-item">
			<a class="nav-link" id="pills-profile-tab" data-toggle="pill" href="#editar" role="tab" aria-selected="false" style="border-radius: 0px;">Editar</a>
		</li>
	</ul>

	<div class="tab-content position-relative" id="pills-tabContent" style="background-color: #131313; margin-top: -16px">
		<div class="tab-pane fade show active" id="visaoGeral" role="tabpanel">
			<div class="visaoGeral">
				<main class="container">
					<header class="profile-header">
						<img src="../<?php echo $srcAvatar;?>" alt="Imagem de perfil">
						<h1><?php echo $row_usuario['apelido'];?></h1>
						<p class="join-date">Membro desde <?php echo $row_usuario['dataConta'];?></p>
						<nav class="profile-nav">
							<ul class="linksList">
								<li><a href="<?php echo $row_usuario['animeList'];?>" target="_blank">Anime List</a></li>
								<li><a href="<?php echo $row_usuario['mangaList'];?>" target="_blank">Mangá List</a></li>
								<li><a href="<?php echo $row_usuario['novelList'];?>" target="_blank">Novel List</a></li>
							</ul>
						</nav>
					</header>
					<section class="fav-works">
						...
						<div class="recomendacao">
							<div class="obras">
								<?php 
									if($linhas == 0) {					
								?>
										<div class="card col-md-12 container clearfix" style="width:350px; margin-top:50px; background:#B22222; color:white;">
											<div class="card-body" style="width:330px; text-align:center;">
												<h5 class="card-title" style="text-align:center">Não há obras favoritadas</h5>
												<center><button class="card-text btBuscaObra" style="margin-top:15px;" onclick="location.href='tudo.php';">Buscar Obras</button><center>
											</div>
										</div>
								<?php	
									}else {
								...
								?>		
											<div class="obra">
												...
												<a href="obras/obras.php?id=<?php echo $imgObra['obras_id'];?>" class="overlay"></a>
											</div>
									<?php 
										}
										}
		...
		<div class="tab-pane fade show" id="editar" role="tabpanel">
			<div class="editar">
				<div style="background-color: #131313">.</div>
				<form id="formPerfil" method="POST" enctype="multipart/form-data" style="width:100%; margin-top: 20px;">
					<input type="hidden" name="id" value="<?php echo $row_usuario['usuario_id'];?>">
					
					<div style='margin-left: 100px;'>
						<label for="AltUname"><b class="altTitulo">Usuário</b></label><br>
						<input type="text" class="inputDados" placeholder="Enter Username" value="<?php echo $row_usuario['apelido'];?>" name="AltUname" required><br>
						
						<label for="AltPsw" style="margin-top: 30px;"><b class="altTitulo">Senha</b></label><br>
						<input type="text" class="inputDados" placeholder="Enter Password" value="<?php echo $row_usuario['senha'];?>" name="AltPsw" required><br>
						
						<label for="AltEmail" style="margin-top: 30px;"><b class="altTitulo">Email</b></label><br>
						<input type="email" class="inputDados" placeholder="Enter Password" value="<?php echo $row_usuario['email'];?>" name="AltEmail" required><br>
					</div>

					<div class="position-absolute" style="margin-top: -244px; margin-left: 430px;">
						<label for="AltAnimeList" style="color: white;"><b class="altTitulo">Anime List</b></label><br>
						<input type="text" class="inputDados" placeholder="URL" value="<?php echo $row_usuario['animeList'];?>" name="AltAnimeList"><br>
					
						<label for="AltMangaList" style="color: white; margin-top: 30px;"><b class="altTitulo">Mangá List</b></label><br>
						<input type="text" class="inputDados" placeholder="URL" value="<?php echo $row_usuario['mangaList'];?>" name="AltMangaList"><br>
						
						<label for="AltNovelList" style="color: white; margin-top: 30px;"><b class="altTitulo">Novel List</b></label><br>
						<input type="text" class="inputDados" placeholder="URL" value="<?php echo $row_usuario['novelList'];?>" name="AltNovelList">
					</div>

					<div class="card-group position-absolute" style="display: block; max-width: 376px; margin-top: -244px; margin-left: 750px;">
						<div class="card border-0" style="display: inline-block; max-width: 150px; background: none;">
							<center><label style="font-size: 20px; color: white;" for="AltImgPerfil"><b>Perfil</b></label></center>
							<img class="preImgAvatar" style="height:150px; width:150px; margin-top: 10px"></img>
							<center><button class="btnUpload" style="margin-top: 20px; border-radius: 10px; color: white; border: 1px solid white;" onclick="document.getElementById('imgAvatarFile').click(); return false;">Upload</button></center>
							<input id= "imgAvatarFile" style="visibility:hidden" type="file" onchange="previewAvatar()" name="AltImgPerfil">
						</div>
						<div class="card border-0" style="display: inline-block; max-width: 150px; margin-left: 60px; background: none;">
							<label style="font-size: 20px; color: white; margin-left: 75px;" for="AltImgWallpaper"><b>Wallpaper</b></label>
							<img class="preImgWallpaper" style="height:150px; width:250px; margin-top: 10px"></img>
							<center><button class="btnUpload" style="margin-top: 20px; border-radius: 10px; color: white; border: 1px solid white; margin-left: 90px;" onclick="document.getElementById('imgWallpaperFile').click(); return false;">Upload</button></center>
							<input id= "imgWallpaperFile" style="visibility:hidden" type="file" onchange="previewWallpaper()" name="AltImgWallpaper">
						</div>	
					</div>

					<div class="card-group" style="margin-top: 50px;">
						<div class="card border-0">
							<button  id="btAlterar" style="border: none;" class="btnAlt" type="submit">Alterar</button>
						</div>
						<div class="card border-0" style="visibility:hidden;"></div>
					</div>
				</form>

				<div class="card-group" style="margin-top: -44px;">
						<div class="card border-0" style="visibility:hidden;"></div>
						<div class="card border-0">
							<button id="btExcluir" data-bs-toggle="modal" data-bs-target="#excluirConta" style="border: none;" class="btnExc" type="submit">Excluir</button>
						</div>
					</div>
				</div>

			<div class="modal fade" id="excluirConta" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content bg-dark">
						<img src="../imagens/contas/excluirConta.png" width="100%" height="388px">
						<form id="formExcluir" method="post">
							<div class="modal-body mx-auto" style="width: 500px; text-align:center; color:white;">
								<h5>Tem certeza que deseja excluir sua conta?</h5>
							</div>
							<div class="modal-footer">
								<button id="btConfirmaExcluir" type="submit" class="btn btn-primary position-relative" style="left: -180px; background-color:#90EE90; width:100px; color:black;">Sim</button>
								<button id="btFechaModal" type="submit" class="btn btn-primary position-relative" data-bs-dismiss="modal" style="left: -80px; background-color:#FF6347; width:100px; color:black;">Não</button>
							</div>
						</form>
					</div>
				</div>
				...
				<script>
					document.getElementById('btAlterar').onclick = function() {
    					document.getElementById('formPerfil').action = '../php/alterarDados.php?id=<?php echo $id?>';
					}

					document.getElementById('btExcluir').onclick = function() {
    					document.getElementById('excluirConta').style.display='block';
					}

					document.getElementById('btConfirmaExcluir').onclick = function() {
    					document.getElementById('formExcluir').action = '../php/excluirConta.php?id=<?php echo $id?>';
					}

					document.getElementById('btFechaModal').onclick = function() {
				...
				<script>
					function previewAvatar(){
						var imgAvatar = document.querySelector('input[name=AltImgPerfil]').files[0];
						var previewImgAvatar = document.querySelector('.preImgAvatar');
		
						var readerAvatar = new FileReader();
		
						readerAvatar.onloadend = function(){
							previewImgAvatar.src= readerAvatar.result;
						}
		
						if(imgAvatar){
							readerAvatar.readAsDataURL(imgAvatar);
						}
						else{
							previewImgAvatar.src="";
						}
					}
		
					function previewWallpaper(){
						var imgWallpaper = document.querySelector('input[name=AltImgWallpaper]').files[0];
						var previewImgWallpaper = document.querySelector('.preImgWallpaper');
		
						var readerWallpaper = new FileReader();
		
						readerWallpaper.onloadend = function(){
							previewImgWallpaper.src= readerWallpaper.result;
						}
		
						if(imgWallpaper){
							readerWallpaper.readAsDataURL(imgWallpaper);
						}
						else{
							previewimgWallpaper.src="";
						}
					}
				</script>
