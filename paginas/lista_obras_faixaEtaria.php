...
// Verifica se a variável faixaEtaria foi passada na URL
if (isset($_GET['faixaEtaria'])) {
    // Recupera o valor da variável faixaEtaria e armazena em $faixaEtaria
    $faixaEtaria = $_GET['faixaEtaria'];
    // Usa o valor de $faixaEtaria para buscar as obras na faixa etária correspondente
    // ...
} else {
    // A variável faixaEtaria não foi passada na URL
    // ...
}

$resultObrasFaixaEtaria = $conn->query("SELECT * FROM Obras WHERE faixaEtaria = '$faixaEtaria'");

$linhasObrasFaixaEtaria = $resultObrasFaixaEtaria->rowCount();

while($row_obras = $resultObrasFaixaEtaria->fetch(PDO::FETCH_ASSOC)){
	$imgsObrasFaixaEtaria[] = $row_obras;
}
?>
...
<body style="visibility: hidden !important; background-color: #171616;">
	...
	<div class="container">
		<?php
			if($faixaEtaria == 0){	
		?>
			<h2 style="color: white;">Livre</h2>
		<?php
			}else {	
		?>
			<h2 style="color: white;">+<?php echo $faixaEtaria;?> Anos</h2>
		<?php 
			}
		?>
		<div class="recomendacao">
			<div class="obras">
				...
				<?php	
						...
						foreach($imgsObrasFaixaEtaria as $obraFaixaEtaria) {
				?>		
							<div class="obra">
								<img src="../<?php echo $obraFaixaEtaria['imagemObra'];?>">
								<h3><?php echo $obraFaixaEtaria['nome'];?></h3>
								<a href="obras/obras.php?id=<?php echo $obraFaixaEtaria['obras_id'];?>" class="overlay">
