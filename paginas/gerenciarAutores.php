...
<head>
	...
	<link rel="stylesheet" type="text/css" href="../css/gerenciarAutores.css"/>
...
<body style="background-color: #171616;">
	<nav class="navbar navbar-expand-lg bg-dark">
		...
		<?php
			if(isset($_SESSION['autor_inserido'])):
		?>
			<body onload="autorAdicionado()"></body>
		<?php
			endif;
			unset($_SESSION['autor_inserido']);
		?>

		<?php
			if(isset($_SESSION['autor_existe'])):
		?>
			<body onload="autorExiste()"></body>	
		<?php
			endif;
			unset($_SESSION['autor_existe']);
		?>

		<?php
			if(isset($_SESSION['autor_removido'])):
		?>
			<body onload="autorRemovido()"></body>
		<?php
			endif;
			unset($_SESSION['autor_removido']);
		?>

        <div class="container">
			<div class="adicionar" onclick="addAutor()">
            <label class="tituloAdd">Adicionar Autores</label>
        </div>	


        <div class="excluir" onclick="delAutor()">
            <label class="tituloExc">Excluir Autores</label>
            </div>
        </div>
</body>
</html>