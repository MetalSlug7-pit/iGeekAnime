...
$resultObrasNomeAutor = $conn->query("SELECT autor_id FROM Autor WHERE nomeAutor = '$nomeAutor'");

$linhasObrasNomeAutor = $resultObrasNomeAutor->rowCount();

while($row_obras = $resultObrasNomeAutor->fetch(PDO::FETCH_ASSOC)){
	$nomeAutor = $row_obras;
}

foreach($nomeAutor as $autores){
	$idAut = $autores;
	$resultAutor = $conn->query("SELECT * FROM Autores_Obra_Autor WHERE idAutor  = '$idAut'");
...
<body style="visibility: hidden !important; background-color: #171616;">
	...
	<div class="container">
		<h2 style="color: white;"><?php echo $autor;?></h2>
		<div class="recomendacao">
			<div class="obras">
				<?php 
					if($linhasObrasNomeAutor == 0) {					
				?>
						<div class="obra">
