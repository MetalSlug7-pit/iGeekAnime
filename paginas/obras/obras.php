...
$obraId = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT);
$result = $conn->query("SELECT * FROM Obras WHERE obras_id = '$obraId'");
$result->execute();
$row_obras = $result->fetch(PDO::FETCH_ASSOC);

$resultVersaoAnime = $conn->query("SELECT versaoAnime FROM Obras WHERE obras_id = '$obraId'");
if($resultVersaoAnime->rowCount()>0){
	$row_anime = $resultVersaoAnime->fetch(PDO::FETCH_ASSOC);
	$anime = $row_anime['versaoAnime'];
}

$resultVersaoManga = $conn->query("SELECT versaoManga FROM OBRAS WHERE obras_id = '$obraId'");
if($resultVersaoManga->rowCount()>0){
	$row_manga = $resultVersaoManga->fetch(PDO::FETCH_ASSOC);
	$manga = $row_manga['versaoManga'];
}

$resultVersaoLN = $conn->query("SELECT versaoLightNovel FROM OBRAS WHERE obras_id = '$obraId'");
if($resultVersaoLN->rowCount()>0){
	$row_lightNovel = $resultVersaoLN->fetch(PDO::FETCH_ASSOC);
	$lightNovel = $row_lightNovel['versaoLightNovel'];
}

$resultVersaoWN = $conn->query("SELECT versaoWebNovel FROM OBRAS WHERE obras_id = '$obraId'");
if($resultVersaoWN->rowCount()>0){
	$row_webNovel = $resultVersaoWN->fetch(PDO::FETCH_ASSOC);
	$webNovel = $row_webNovel['versaoWebNovel'];
}

$idGeneros = null;

$resultIdGeneros = $conn->query("SELECT idGenero FROM Generos_Obra_Genero where idObra = '$obraId'");
$linhas = $resultIdGeneros->rowCount();
while($row_idGeneros = $resultIdGeneros->fetch(PDO::FETCH_ASSOC)){
	$idGeneros[] = $row_idGeneros;
}
...
if($idGeneros==null){
	...
	foreach($idGeneros as $generos){
		$idGenero = $generos['idGenero'];
		$resultGenero = $conn->query("SELECT * FROM Genero WHERE genero_id = '$idGenero'");
		while($row_generos = $resultGenero->fetch(PDO::FETCH_ASSOC)){
			$genero[] = $row_generos;
		}
	}
}
...
$resultIdAutores = $conn->query("SELECT idAutor FROM Autores_Obra_Autor where idObra = '$obraId'");
$linhas = $resultIdAutores->rowCount();
while($row_idAutores = $resultIdAutores->fetch(PDO::FETCH_ASSOC)){
	$idAutores[] = $row_idAutores;
}
...
if($idAutores==null){
	...
	foreach($idAutores as $autores){
		$idAutor = $autores['idAutor'];
		$resultAutor = $conn->query("SELECT * FROM Autor WHERE autor_id = '$idAutor'");
		while($row_autores = $resultAutor->fetch(PDO::FETCH_ASSOC)){
			$autor[] = $row_autores;
		}
	}
}
...
<body style="visibility: hidden !important; background-color: #171616;">
	...
	<div class="modal fade" id="loginModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<img src="../../imagens/contas/avatarLogin.png" width="100%" height="388px">
	...
	<div class="modal fade" id="cadastroModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<img src="../../imagens\contas/avatarCadastro.png">
	...
	<img style="width: 100%; height: 658px;" src="../../<?php echo $row_obras['wallpaperObra'];?>">

	<ul class="nav nav-pills mb-3 nav-fill position-relative" id="pills-tab" role="tablist" style="background-color: #131313;">
		<li class="nav-item">
		  <a class="nav-link active" id="pills-home-tab" data-toggle="pill" href="#visaoGeral" role="tab" aria-selected="true" style="border-radius: 0px;">Visão Geral</a>
		</li>
		<!--<li class="nav-item">
		  <a class="nav-link" id="pills-profile-tab" data-toggle="pill" href="#versoes" role="tab" aria-selected="false" style="border-radius: 0px;">Versões</a>
		</li>
		<li class="nav-item">
			<a class="nav-link" id="pills-home-tab" data-toggle="pill" href="#redesSociais" role="tab" aria-selected="true" style="border-radius: 0px;">Redes Sociais</a>
		</li>-->
	</ul>

	<div class="tab-content position-relative" id="pills-tabContent">
		<div class="tab-pane fade show active" id="visaoGeral" role="tabpanel" style="margin-top: 40px;">
			<div class="titulo" style="color: white; margin-left: 50px; font-size: 30px" id="txtTituloObra" disabled><?php echo $row_obras['nome'];?> 
				<?php 
					if(isset($_SESSION['usuario'])):
				?>
				<div class="formFavorito">
					<form style="border:none;" method="POST" action="../../php/favoritarObra.php">
						<input type="hidden" name="idUsuario" value="<?php echo $idUsuario;?>">
						<input type="hidden" name="idObra" value="<?php echo $row_obras['obras_id'];?>">
						<button id="btFav" type="submit" style="background:none; border:none; margin-top:-100px;">
							<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-heart" width="34" height="34" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ff2825" fill="none" stroke-linecap="round" stroke-linejoin="round" style="margin-top:-10px;">
							<path stroke="none" d="M0 0h24v24H0z" fill="none"/>
				...
				<div class="formDesfavorito">
					<form style="border:none;" method="POST" action="../../php/desfavoritarObra.php">
						<input type="hidden" name="idUsuario" value="<?php echo $idUsuario;?>">
						<input type="hidden" name="idObra" value="<?php echo $row_obras['obras_id'];?>">				
						<button id="btDesFav" type="submit" style="background:none; border:none;">
							<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-heart" width="34" height="34" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ff2825" fill="#ff2825" stroke-linecap="round" stroke-linejoin="round" style="margin-top:-10px;">
							<path stroke="none" d="M0 0h24v24H0z" fill="none"/>
							<path d="M19.5 13.572l-7.5 7.428l-7.5 -7.428m0 0a5 5 0 1 1 7.5 -6.566a5 5 0 1 1 7.5 6.572" />
							</svg>
						</button>
			...
			<?php
				$idObra = $row_obras['obras_id'];
				$sql = "SELECT * FROM Favoritar_Obra_Usuario where idUsuario = '$idUsuario' and idObra = '$idObra'";
				$result = $conn->query($sql);
						
				if($result->rowCount() > 0) {
					$_SESSION['obra_favoritada'] = TRUE;
				}

				if(isset($_SESSION['obra_favoritada'])){
			?>
					<script>
						document.getElementById('btFav').style.display='none';
						document.getElementById('btDesFav').style.display='block';
					</script>
			<?php
				}
				else{				
			?>
					<script>
							document.getElementById('btFav').style.display='block';
							document.getElementById('btDesFav').style.display='none';
					</script>
			<?php
				}
				unset($_SESSION['obra_favoritada']);
			?>

			<?php
				if(isset($_SESSION['obra_favoritada_sucesso'])):
			?>
				<body onload="obraFavoritada()"></body>
			<?php
				elseif(isset($_SESSION['obra_desfavoritada_sucesso'])):
			?>
				<body onload="obraDesfavoritada()"></body>
			<?php
				endif;
				unset($_SESSION['obra_favoritada_sucesso']);
				unset($_SESSION['obra_desfavoritada_sucesso']);
			?>

			<div class="sinopse">
				<p class="txtArea"><?php echo $row_obras['sinopse'];?></p>
			</div>

			<div class="faixaEtaria">
				<h5 style="color: white; margin-left: 50px; margin-top: 30px">
					<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-ban" width="30" height="30" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
				...
				<?php 
					if($row_obras['faixaEtaria']== 0){
				?>
				<div style="margin-left: 20px; margin-top: 10px">
					<div class="linkFaixaEtaria"><center><a class="linkFaixaEtariaTxt" href="../lista_obras_faixaEtaria.php?faixaEtaria=<?php echo $row_obras['faixaEtaria'];?>">Livre</a></center></div>
				</div>
				...
				?>
				<div style="margin-left: 20px; margin-top: 10px">
					<div class="linkFaixaEtaria"><center><a class="linkFaixaEtariaTxt" href="../lista_obras_faixaEtaria.php?faixaEtaria=<?php echo $row_obras['faixaEtaria'];?>">+<?php echo $row_obras['faixaEtaria'];?> Anos</a></center></div>
				</div>
			</div>
			<?php 
					}
			...
			<div class="genero">
				<h5 style="color: white; margin-left: 50px; margin-top: 30px">
					<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-archive" width="30" height="30" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round" stroke-linejoin="round">
						<path stroke="none" d="M0 0h24v24H0z" fill="none"/>
				...
				<div style="margin-left: 20px; margin-top: 10px">
					<?php 
						if($linhas==0){
					?>
							<button class="linkGenero">Sem Genero</button>
					<?php	
						}else{
							foreach($genero as $generoObra){
					?>
								<div class="linkGenero"><center><a class="linkGeneroTxt" href="../lista_obras_genero.php?tipoGenero=<?php echo $generoObra['tipoGenero'];?>"><?php echo $generoObra['tipoGenero'];?></a></center></div>
					<?php
							}
						}
					?>
			...
			<div class="autores">
				<h5 style="color: white; margin-left: 50px; margin-top: 30px">
					<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-address-book" width="30" height="30" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
						<path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
				...
				<div style="margin-left: 20px; margin-top: 10px">
					<?php 
						if($linhas==0){
					?>
							<button class="linkAutor">Sem Autor</button>
					<?php	
						}else{
							foreach($autor as $autorObra){
					?>
								<div class="linkAutor"><center><a class="linkAutorTxt" href="../lista_obras_autor.php?nomeAutor=<?php echo $autorObra['nomeAutor'];?>"><?php echo $autorObra['nomeAutor'];?></a></center></div>
						<?php
							}
						}
						?>
			...
			<div class="versoes" style="display: block;">
				<h5 style="color: white; margin-left: 50px; margin-top: 30px;">
					<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-versions" width="30" height="30" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round" stroke-linejoin="round">
						<path stroke="none" d="M0 0h24v24H0z" fill="none"/>
				...
				<div style="margin-left: 10px; margin-top: 20px;">
					<?php 
					if ($anime || $manga || $lightNovel || $webNovel) {
						if ($anime) {
							?>
							<div>
								<div class="styleVersoes linkVersoes" style="margin-left: 55px; display: inline-block; height: 65px;">
									<center><a href="<?php echo $anime ?>" target="_blank">
										<svg class="versaoIcone" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-player-play" width="40" height="40" viewBox="0 0 24 24" stroke-width="1" fill="none" stroke-linecap="round" stroke-linejoin="round">
											<path stroke="none" d="M0 0h24v24H0z" fill="none"/>
											<path d="M7 4v16l13 -8z" />
										</svg>
										<p class="versaoTitulo">Anime</p>
									</a></center>
								</div>
							<?php
						}
						if ($manga) {
							?>
								<div class="styleVersoes linkVersoes" style="margin-left: 55px; display: inline-block; height: 65px;">
									<center><a href="<?php echo $manga ?>" target="_blank">
										<svg class="versaoIcone" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-message-circle-2" width="40" height="40" viewBox="0 0 24 24" stroke-width="1" fill="none" stroke-linecap="round" stroke-linejoin="round">
											<path stroke="none" d="M0 0h24v24H0z" fill="none"/>
						...
						if ($lightNovel) {
							?>
								<div class="styleVersoes linkVersoes" style="margin-left: 55px; display: inline-block; height: 65px;">
									<center><a href="<?php echo $lightNovel ?>" target="_blank">
										<svg class="versaoIcone" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-notebook" width="40" height="40" viewBox="0 0 24 24" stroke-width="1" fill="none" stroke-linecap="round" stroke-linejoin="round">
											<path stroke="none" d="M0 0h24v24H0z" fill="none"/>
											<path d="M6 4h11a2 2 0 0 1 2 2v12a2 2 0 0 1 -2 2h-11a1 1 0 0 1 -1 -1v-14a1 1 0 0 1 1 -1m3 0v18" />
											<line x1="13" y1="8" x2="15" y2="8" />
											<line x1="13" y1="12" x2="15" y2="12" />
											</svg>
										<p class="versaoTitulo">Light Novel</p>
									</a></center>
								</div>
							<?php 
						}
						if ($webNovel) {
							?>
								<div class="styleVersoes linkVersoes" style="margin-left: 55px; display: inline-block; height: 65px;">
									<center><a href="<?php echo $webNovel ?>" target="_blank">
											<svg class="versaoIcone" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-browser" width="40" height="40" viewBox="0 0 24 24" stroke-width="1" fill="none" stroke-linecap="round" stroke-linejoin="round">
												<path stroke="none" d="M0 0h24v24H0z" fill="none"/>
												<rect x="4" y="4" width="16" height="16" rx="1" />
												<line x1="4" y1="8" x2="20" y2="8" />
												<line x1="8" y1="4" x2="8" y2="8" />
											</svg>
										<p class="versaoTitulo">Web Novel</p>
									</a></center>
								</div>
							</div>	
				</div>	
			...
			<!--<div class="redesSociais">
				<h5 style="color: white; margin-left: 50px; margin-top: 50px;">
					<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-share" width="30" height="30" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round" stroke-linejoin="round">
						<path stroke="none" d="M0 0h24v24H0z" fill="none"/>
		...
		<!--<div class="tab-pane fade" id="versoes" role="tabpanel">
			B
		</div>

		<div class="tab-pane fade" id="personagens" role="tabpanel">
			C
		</div>
		...
		<div class="tab-pane fade" id="redesSociais" role="tabpanel">
			E
		</div>-->
	</div>
